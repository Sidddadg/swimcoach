...
export async function POST() {
  try {
    ...
    if (existingUsers > 0) {
      ...
      return NextResponse.json({ seeded: false, message: "Already seeded", users });
    }
    ...
    for (const a of bobActivities) {
      ...
      await prisma.activity.create({
        data: {
          ...
          durationSecs,
          source: a.source,
          poolLength: a.isOpenWater ? null : 25,
    ...
    return NextResponse.json({
      seeded: true,
      users: [{ id: alice.id, name: alice.name }, { id: bob.id, name: bob.name }],
    }, { status: 201 });
  } catch (err) {
